...
CRITICAL: Write the visible HTML body content first. Use mostly inline styles. Keep any <style> block under 30 lines. If you run out of space, cut CSS — never cut body content. A page with simple styles and real content is better than beautiful CSS with an empty body.`
...
[{"path":"index.html","content":"<!DOCTYPE html>...complete html..."}]`
  ...
  // Salvage: extract HTML directly from the raw string (works even when JSON is truncated mid-content)
  if (!files) {
    const htmlStart = result.indexOf('<!DOCTYPE')
    if (htmlStart !== -1) {
      let html = result.slice(htmlStart)
      ...
      html = html
        ...
        .replace(/\\"/g, '"')
        .replace(/\\\\/g, '\\')
      // Fix unclosed <style> block — entire body would be invisible otherwise
      if (html.includes('<style') && !html.includes('</style>')) {
        // Find the last complete CSS rule (closing brace) and close the style block there
        const lastBrace = html.lastIndexOf('}')
        if (lastBrace !== -1) {
          html = html.slice(0, lastBrace + 1) + '\n</style>\n<body>\n<p style="font-family:sans-serif;padding:32px">Prototype is loading...</p>\n'
        } else {
          html += '\n</style>\n<body>\n'
        }
      }
      ...
      console.log(`[generate] Salvaged ${html.length} chars of HTML from truncated response`)
    }
  }
