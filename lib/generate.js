...
Return a JSON array of ALL files needed to run this app. Include:
- package.json
- next.config.ts
- tailwind.config.ts
- postcss.config.mjs
...
- app/page.tsx
- app/globals.css
- Any additional pages and components needed

Format: [{"path": "package.json", "content": "..."}, ...]
...
Return ONLY the JSON array, no other text.`
  ...
  console.log(`[generate] Response: stop_reason=${response.stop_reason}, content blocks=${response.content.length}`)

  result = response.content.filter(b => b.type === 'text').map(b => b.text).join('')
  console.log(`[generate] Text length: ${result.length} chars`)
