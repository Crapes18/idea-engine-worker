...
// ── Color palettes — each template type gets a distinct look ─────────────────
export const PALETTES = {
  flashcard: [
    { primary: '#0F766E', accent: '#6366F1' },  // teal + indigo
    { primary: '#7C3AED', accent: '#EC4899' },  // purple + pink
    { primary: '#1D4ED8', accent: '#F59E0B' },  // blue + amber
  ],
  quiz:      [
    { primary: '#FF6B6B', accent: '#7C3AED' },  // coral + purple (current)
  ],
  landing:   [
    { primary: '#1A1A1A', accent: '#6366F1' },  // dark + indigo
    { primary: '#0369A1', accent: '#0F766E' },  // sky + teal
    { primary: '#7C3AED', accent: '#EC4899' },  // purple + pink
  ],
}

export function pickPalette(type, seed) {
  const set = PALETTES[type] || PALETTES.landing
  // Use seed (idea name hash) to pick deterministically but variably
  let h = 0
...
export async function generateFlashcardContent({ name, brief, founderNotes, founderAvoid }) {
  const context = [
    brief?.target_customer ? `Target user: ${brief.target_customer}` : null,
    brief?.mvp_scope ? `MVP: ${brief.mvp_scope}` : null,
    founderNotes ? `Founder direction: ${founderNotes}` : null,
  ...
  const palette = pickPalette('flashcard', name)

  const prompt = `Generate flashcard deck content for an app called "${name}".

Context:
${context}

Return ONLY valid JSON with this exact structure. No apostrophes — use "cannot" not "can't". Double quotes only.

{
  "app": {
    "title": "${name}",
    "subtitle": "One-line description of what users learn",
    "emoji": "🃏",
  ...
  "decks": [
    {
      ...
      "emoji": "📚",
      "cards": [
        {"front": "Question or term", "back": "Answer or definition", "hint": "Optional memory aid"},
        {"front": "...", "back": "...", "hint": ""}
      ]
    }
  ]
...
Rules:
- Create 3-5 decks relevant to the app topic
- Each deck should have 6-10 cards
- Cards should be genuinely useful for the target user
- Make fronts concise (questions, terms, prompts)
- Make backs clear and informative
- Hints are optional but helpful
- Emoji should match the deck topic`

  console.log('[content-gen] Generating flashcard content with Claude...')
  try {
    const response = await anthropic().messages.create({
      model: 'claude-sonnet-4-6',
      max_tokens: 4096,
    ...
    if (match) {
      const content = JSON.parse(match[0])
      console.log(`[content-gen] Flashcard content generated: ${content.decks?.length} decks`)
      return content
    }
  } catch (e) {
    console.warn('[content-gen] Claude generation failed, using fallback:', e.message)
  ...
  // Fallback content
  return {
    app: { title: name, subtitle: 'Learn and memorize with flashcards', emoji: '🃏', primary: palette.primary, accent: palette.accent },
    decks: [
      {
        id: 'deck1', title: 'Getting Started', description: 'Introduction to the basics', emoji: '📖',
        cards: [
          { front: 'What is this app for?', back: 'Learning and memorizing key concepts through flashcards', hint: '' },
          { front: 'How do you study?', back: 'Tap a card to reveal the answer, then mark if you got it right', hint: '' },
          { front: 'What happens when you finish a deck?', back: 'You see your score and can study again to improve', hint: '' },
        ]
      }
    ]
  }
